...
if(isset($_POST['add_event'])){
    ...
    if(mysqli_query($conn, $insert)){
        // Instead of breaking design with browser popups, we pass a success parameter
        header("Location: admin_events.php?status=success");
        exit();
    }else{
        $error_msg = "Database Error: Unable to publish your event session.";
    }
}
?>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="title" placeholder="e.g., UI/UX Design Masterclass Workshop" required
                               class="w-full text-sm bg-slate-50 border border-slate-200 rounded-xl pl-4 pr-4 py-3 focus:bg-white focus:ring-2 focus:ring-blue-600/15 focus:border-blue-600 transition outline-none text-slate-800 font-medium placeholder:text-slate-400">
                    </div>
                </div>
